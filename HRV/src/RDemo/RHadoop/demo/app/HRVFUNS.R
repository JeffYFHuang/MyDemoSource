...
caculateHRV <- function(beats, windowsize = 300) {
    result = list()
 ...
 #   hrv.data$Beat = filterHR(hrv.data)
       ...
       hrv.data = CreateTimeAnalysis(hrv.data, size = 100, interval = 7.8125) #size = 100
...
getHRVData <- function(beats, windowsize = 300, shift = 300, mod.fit = NULL) {
    winsBeatData = getWindowsBeatData(beats, windowsize = 300, shift = 300)
    winsBeatData = winsBeatData[lengths(winsBeatData) > 0]
    HRVData = lapply(winsBeatData, caculateHRV, windowsize = windowsize)
    return(HRVData)
}

getHRVFeatures <- function(HRVData, windowsize = 300, shift = 300, mod.fit = NULL) {
    #beatLists = SplitBeatDataByDuration(HRVData)
    results = list()
    results$shift = shift
    results$windowsize = windowsize
    #results$CaculatedData = NULL
    #results$CaculatedData = lapply(beatLists, getHRVData, windowsize = 300, shift = 300, mod.fit = NULL)
    #if (is.list(unlist(results$CaculatedData, recursive = FALSE)))
    #   results$CaculatedData = unlist(results$CaculatedData, recursive = FALSE)a
    results$CaculatedData = getHRVData(HRVData$Beat$Time)
    results$CaculatedData = results$CaculatedData[lengths(results$CaculatedData) > 0]
    return(results)
}
...
JsonToDataFrame <- function (jsonText = NULL) {
   ...
   size = length(unlist(jsonData$CaculatedData))
   row.size = length(unlist(jsonData$CaculatedData[[1]]))
   col.names = names(jsonData$CaculatedData[[1]])
   HRV.Features = t(array(as.numeric(unlist(jsonData$CaculatedData)), c(row.size, size/row.size)))
   HRV.Features = data.frame(HRV.Features)
   colnames(HRV.Features) = col.names
   return(HRV.Features)
}
